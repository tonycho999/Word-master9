...
const WordGuessGame = () => {
  ...
  // setMessage를 gameLogic에 전달하여 힌트 메시지 제어
  ...
  const handleSendOtp = async (e) => {
    ...
    setIsLoading(true); playSound('click');
    const { error } = await supabase.auth.signInWithOtp({ email: inputEmail });
    setIsLoading(false);
    if (error) { auth.setMessage(error.message.includes('rate limit') ? 'Wait a moment...' : 'Error sending code'); } 
    else { setIsOtpSent(true); auth.setMessage('Code sent to email!'); }
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setIsLoading(false);
    if (error) { auth.setMessage('Wrong Code. Try again.'); } 
    else { auth.setMessage('LOGIN SUCCESS!'); auth.setShowLoginModal(false); setIsOtpSent(false); setOtp(''); }
    setTimeout(() => auth.setMessage(''), 3000);
  };

  const closeLoginModal = () => {
      auth.setShowLoginModal(false); setIsOtpSent(false); setOtp(''); setInputEmail('');
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-screen w-full bg-indigo-600 p-4 font-sans text-gray-900 select-none relative">
              ...
              <div className="bg-white rounded-2xl p-6 max-w-xs w-full shadow-2xl animate-fade-in-up">
                  <div className="flex justify-between items-center mb-4">
                      <h3 className="text-xl font-black text-indigo-600 flex items-center gap-2">
                        {isOtpSent ? <Key size={24}/> : <Mail size={24}/>} {isOtpSent ? 'VERIFY CODE' : 'LOGIN'}
                      </h3>
                  ...
                  {!isOtpSent ? (
                      <form onSubmit={handleSendOtp} className="flex flex-col gap-3">
                          ...
                          <button type="submit" disabled={isLoading} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-black flex items-center justify-center gap-2 hover:bg-indigo-700 disabled:opacity-50">{isLoading ? 'SENDING...' : 'SEND CODE'} <Send size={16}/></button>
                      </form>
                  ) : (
                      <form onSubmit={handleVerifyOtp} className="flex flex-col gap-3">
                          <p className="text-xs text-center text-gray-500 font-bold">Enter the 6-digit code sent to<br/><span className="text-indigo-600">{inputEmail}</span></p>
                          ...
                          <button type="submit" disabled={isLoading} className="w-full py-3 bg-green-600 text-white rounded-xl font-black flex items-center justify-center gap-2 hover:bg-green-700 disabled:opacity-50">{isLoading ? 'VERIFYING...' : 'VERIFY & LOGIN'} <Key size={16}/></button>
                          <button type="button" onClick={() => { setIsOtpSent(false); setOtp(''); }} className="text-xs text-gray-400 font-bold flex items-center justify-center gap-1 hover:text-gray-600 mt-2"><ArrowLeft size={12}/> Change Email</button>
                      </form>
                  )}
              </div>
          </div>
      ...
      <div className="bg-white rounded-[2rem] p-4 w-full max-w-md shadow-2xl flex flex-col items-center border-t-8 border-indigo-500">
        ...
        <GameControls 
            ...
            // 힌트 버튼 텍스트 설정 (3단계는 'SHOW SPACES'로 설정)
