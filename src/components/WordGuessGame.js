    ...
    <div className="flex flex-col items-center justify-center min-h-screen w-full bg-indigo-600 p-4 font-sans relative">
      ...
      {showInterstitial && (
        <div className="absolute inset-0 z-50 bg-black flex flex-col items-center justify-center p-10 text-white text-center">
          ...
          <div className="text-xl font-bold mb-4 italic">ADVERTISING...</div>
      ...
      <div className="bg-white rounded-[2.5rem] p-6 sm:p-10 w-full max-w-md shadow-2xl flex flex-col items-center border-t-8 border-indigo-500 mx-auto">
        ...
        {/* 상단 버튼바: 모든 아이콘(Lightbulb, RotateCcw) 사용됨 */}
        <div className="flex gap-2 mb-8">
          <button 
            onClick={() => { playSound('click'); setScore(s => s - 100); setHintLevel(1); }} 
            disabled={score < 100 || hintLevel > 0 || isCorrect} 
            className="px-4 py-2 bg-gray-100 rounded-full text-[10px] font-black flex items-center gap-1 uppercase"
          >
            <Lightbulb size={12}/> {hintLevel > 0 ? currentWord[0] : 'Hint (-100)'}
          </button>
          <button 
            onClick={() => { playSound('click'); setScrambledLetters(p => [...p].sort(() => Math.random() - 0.5)); }} 
            className="px-4 py-2 bg-gray-100 rounded-full text-[10px] font-black flex items-center gap-1 uppercase"
          >
            <RotateCcw size={12}/> Shuffle
          </button>
          <button 
            onClick={() => { setIsAdLoading(true); setTimeout(() => { setScore(s => s + 200); setIsAdLoading(false); playSound('success'); }, 3000); }} 
            className="px-4 py-2 bg-amber-400 text-white rounded-full text-[10px] font-black shadow-sm flex items-center gap-1"
          >
            <PlayCircle size={12}/> {isAdLoading ? '...' : 'FREE 200P'}
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-10 min-h-[60px]">
          {scrambledLetters.map(l => (
            <button key={l.id} onClick={() => { playSound('click'); setSelectedLetters(p => [...p, l]); setScrambledLetters(p => p.filter(i => i.id !== l.id)); }} className="w-11 h-11 bg-white border-2 border-gray-100 rounded-xl font-black text-lg shadow-sm hover:border-indigo-400">
              {l.char.toUpperCase()}
            </button>
          ))}
        </div>
        ...
        <div className={`w-full min-h-[140px] rounded-[2rem] flex flex-col justify-center items-center p-6 mb-8 border-2 border-dashed ${isCorrect ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100'}`}>
          ...
          {isCorrect && <div className="text-green-500 font-bold mt-4 animate-bounce">{message}</div>}
        </div>

        {/* 하단 제어부 */}
        <div className="w-full">
          {isCorrect ? (
            <button onClick={() => level % 10 === 0 ? setShowInterstitial(true) : processNextLevel()} className="w-full bg-green-500 text-white py-5 rounded-[2rem] font-black text-2xl shadow-lg flex items-center justify-center gap-2">
